...
%             DScopy = mysort.mea.compoundMea(h5FileList, 'useFilter', 0, 'name', 'PREFILT');            
            ...
            [S P_] = mysort.sorters.sort(DScopy, fullfile(outPath, ['group' sprintf('%03d', ii)]), runName, P);
            % RELEASE CHANNEL RESTRICTIONS FOR TEMPLATE ESTIMATION
            DScopy.restrictToChannels();
            ana.startHDSortingTemplateEstimation(outPath, ['group' sprintf('%03d', ii)], runName, Tf, cutleft, DScopy, groupsidx{ii}, MES);
        end
    else
        for ii=1:length(groupsidx)
            DSFull.restrictToChannels(groupsidx{ii});
            [S P_] = mysort.sorters.sort(DSFull, fullfile(outPath, ['group' sprintf('%03d', ii)]), runName, P);
            % RELEASE CHANNEL RESTRICTIONS FOR TEMPLATE ESTIMATION
            DSFull.restrictToChannels();
            ana.startHDSortingTemplateEstimation(outPath, ['group' sprintf('%03d', ii)], runName, Tf, cutleft, DSFull, groupsidx{ii}, MES);
        end
    end
    ...
    [gdf_merged T_merged localSorting localSortingID] =...
        ana.processLocalSortings(outPath, runName, groups, groupsidx);
