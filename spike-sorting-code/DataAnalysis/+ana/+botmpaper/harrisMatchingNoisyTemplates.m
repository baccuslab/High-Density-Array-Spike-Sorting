...
nRep = 10;
percentageSpikesPerTemplate = [100:-10:10];
nSpikesPerTemplate = [2:5:50];
Sensitivity = [];
Specificity = [];
Classification = [];
T1 = [];
T2 = [];
for nd = 1:length(nSpikesPerTemplate)
    d = nSpikesPerTemplate(nd);
    for r = 1:nRep
        % Compute Templates with a subset of all spikes that were assigned
        % to them
        Tcorrect_sorted_recut_this = [];
        for i=1:length(U)
            S = correctSpikes_sorted_recut{i};
            selIdx = randperm(size(S,1), d);
            Tcorrect_sorted_recut_this(i,:) = mean(S(selIdx,:));    
        end
        if nd==1 && r==1
            T1 = Tcorrect_sorted_recut_this;
        elseif nd==length(percentageSpikesPerTemplate) && r==1
            T2 = Tcorrect_sorted_recut_this;
        end
            
        RR = ana.botmpaper.harrisMatchingEvalFun(Tcorrect_sorted_recut_this, allSpikes, correctSpikes_sorted_recut{correctIdx}, noiseSnippets, C);
        
        [mx mxidx] = max(RR.botmCor,[],2);
        nClassified = sum(mxidx==correctIdx & mx > 0);
        nCorrectSpikes = length(mxidx);
        classPerfBotm = nClassified/nCorrectSpikes;
        % This is correct. TP/(TP+CNt) = TP / TotalNumberOfCorrectSpikes
        ...
        rejPerfBotm = (sum(mxidx==correctIdx & mx > 0)- nClassified)/(length(mxidx)-nCorrectSpikes);
...
nC = size(DS,2);
save('harrisMatchingNoisyTemplatesBuffer2.mat', 'T1', 'T2', 'Sensitivity', 'Classification', 'Specificity',...
    'nSpikesPerTemplate', 'correctIdx', 'nC');
