global savefig__;
savefig__ = 1;
[CLASSPERF CPUTIMEPERF DETPERF L] = ana.botmpaper.loadQuirogaEval();
%%
    mysort.plot.figure('w',1100, 'h', 800);
    ah = subplot(4,1,1); hold on
    ...
    ylabel(ah(2), 'Class Perfo [%]', 'fontsize', 12);
     ...
     ah(3) = subplot(4,1,3); hold on
     PP = (CLASSPERF + DETPERF)/2;
    mRP = squeeze(mean(PP));
    sRP = squeeze(std(PP));
    handles = matlabfilecentral.barweb.barweb(mRP, sRP, [], [], [], [], [], bone, [], [], 1, 'plot');
    ylabel(ah(3), 'Total Perf [%]', 'fontsize', 12);
    set(ah(3), 'ylim', [min(mRP(:)) 100]);
    set(ah(3), 'xticklabel', L.datasetNames(correctSpikesIdx))   
    
     ah(4) = subplot(4,1,4); hold on
     PP = CPUTIMEPERF;
    mRP = squeeze(mean(PP));
    sRP = squeeze(std(PP));
    handles = matlabfilecentral.barweb.barweb(mRP, sRP, [], [], [], [], [], bone, [], [], 1, 'plot');
    ylabel(ah(4), 'CPU Time [s]', 'fontsize', 12);
    set(ah(4), 'xticklabel', L.datasetNames(correctSpikesIdx))      
    mysort.plot.savefig(gcf, 'BigPerformanceAllProcessingPlotQuiroga2', 'fig', 0)
