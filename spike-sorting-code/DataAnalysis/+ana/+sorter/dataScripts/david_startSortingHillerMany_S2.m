...
for i=1:length(h5FileLists)
    configFolderPathList{i} = fullfile(outPath, ['Config' num2str(i)]);
    sortOutPath = fullfile(configFolderPathList{i}, sortingName);
...
%% 
configIdx = 2;
[SSC S compoundMea G] = ana.toni.inspectSorting(sortingName, configFolderPathList{configIdx}, h5FileLists{configIdx});
...
% showUnitsIdx = [1 3 4 5 6 7 8 9 10 11];
% showUnitsIdx = [12:30];
% showUnitsIdx = [60:90];
 showUnitsIdx = [2:10:90];
%  showUnitsIdx = 51;
