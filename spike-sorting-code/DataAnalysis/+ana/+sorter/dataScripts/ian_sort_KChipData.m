...
ntkpath = fullfile(pd.serverDataRoska, '..', 'jaeckeld', '2013_12_03_Ian_RGC_K_chip', 'proc');

expName = '2013_12_03_Ian_RGC_K_chip';
sortingName = 'Sorting1';

outPath = fullfile(pd.serverData, 'Ian', [expName 'Out']);

if ~exist(outPath, 'file'); mkdir(outPath); end

configNTKLists = {};
fnames = dir(fullfile(ntkpath, '*.ntk'));
for i=1:length(fnames)
    configNTKLists{i} = {fullfile(ntkpath, fnames(i).name)};
end
...
save(fullfile(outPath, [expName '_sortedSpikeTrains.mat']), 'R', 'configNTKLists', 'h5FileLists');
% end
% 
% 
% %% 
