...
%%
save('Sorting2.mat', 'BM', 'SCall', 'EL', 'ELNR');

%%
elIdx = 8:17; 
D1 = load('Sorting1.mat');
D2 = load('Sorting2.mat');
D2.BM(:,elIdx) = D2.BM(:,elIdx) + size(D1.BM,1);

BM = [D1.BM; D2.BM];
SCall = [D1.SCall; D2.SCall];
ELPOS = [D1.EL; D2.EL];
ELNR = [D1.ELNR; D2.ELNR];
...
idxStable = SCall(:,1) > SCall(:,2) & SCall(:,3) > SCall(:,2); %BM(:,7) > .5 & BM(:,7) < 1.5; % Mod 3
idxMod1up   = BM(:,5) > 1 + .1; % Mod 1
idxMod1down = SCall(:,1) > SCall(:,2)*1.3 & SCall(:,3) > SCall(:,2)*1.3 & SCall(:,1) > 10 & SCall(:,3) > 10; % Mod 1
nR = 1;
plots = {'Amp' idxAmp
         'Amp + Stabil' idxAmp & idxStable
         'Amp + Mod1 hoch' idxAmp & idxMod1up
...
%%
ELs4Michele = [];
ELs4Michele(:,:,1) = reshape(ELPOS(sBM(:,elIdx),1), [size(sBM,1) length(elIdx)]);
ELs4Michele(:,:,2) = reshape(ELPOS(sBM(:,elIdx),2), [size(sBM,1) length(elIdx)]);
ELs4Michele(:,:,3) = reshape(ELNR(sBM(:,elIdx),1),  [size(sBM,1) length(elIdx)]);
