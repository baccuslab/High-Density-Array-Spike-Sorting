...
for i = 1:length(GE)
    g = GE(i);
    this = [ones(length(g.Units(:)), 1)*i g.Units(:) g.TotalSpikeCount(:) g.goodElectrodesAmplitudesMax(:) g.ModulationIndex1(:) g.ModulationIndex2(:) g.ModulationIndex3(:) g.goodElectrodes(:,1:20) + nEl];
    BM = [BM; this];
    EL = [EL; g.elPos];
...
%%
mysort.plot.figure([1200 600])
idxAmp = BM(:,4) > 40; % Amplitude
idxStable = BM(:,7) > .5 & BM(:,7) < 1.5; % Mod 3
idxMod1up   = BM(:,5) > 1 + .1; % Mod 1
idxMod1down = BM(:,5) < 1 - .1; % Mod 1

plots = {'Amp + Stabil' idxAmp & idxStable
