...
ntk = struct();
ntk.data = mysort.h5.matrix(ffile, P.h5path, true);
ntk.chipid = -1;
ntk.gain1 = 1;
ntk.gain2 = 1;
ntk.gain3 = 1;
...
ord = mysort.h5.createVariableAndOrFile(P.outFile, [P.sessionName '/source_files/raw_h5'], [1 length(ffile)], [1 length(ffile)], 'H5T_C_S1');
ord(1,1:length(ffile)) = ffile;
clear ord
mfile = map.file;
ord = mysort.h5.createVariableAndOrFile(P.outFile, [P.sessionName '/source_files/mapping_file'], [1 length(mfile)], [1 length(mfile)], 'H5T_C_S1');
...
while lastSample < nSamples
    ...
    X_ = ntk.data(lastSample+1:thisLastSample, [isConnectedIdx; P.frameNoIdx']);
    X = X_(:, 1:end-2);
    Y = double(X_(:, end-1:end));
    %X = ntk.data(lastSample+1:thisLastSample, isConnectedIdx);
    %Y = double(ntk.data(lastSample+1:thisLastSample, P.frameNoIdx));
    FRAMES(lastSample+1:thisLastSample) = bitor( bitshift( Y(:,2) , 16 ) , Y(:,1) );
...
%F = mea1k.file(ffile); F.getH5Info();
%FRAMES = F.getFrameNo();
df = [diff(FRAMES)-1; 0];
idx = find(df);
firstMissingFR = FRAMES(idx)+1;
L   = df(idx);
lL = length(L);
...
if 0
    ...
    L   = df(idx)
    lL = length(L)
    ffn(:,:) = [firstMissingFR(:) L(:)];
    
    FRAMES_RESTORED = mysort.mea.getFrameNumbersFromMissing(FRAMES(1), FRAMES(end), ffn)
end

ffn = mysort.h5.createVariableAndOrFile(P.outFile, [P.sessionName '/frame_numbers/first_fn'], [1 1], [1 1], 'H5T_NATIVE_INT');
ffn(1,1) = int32(FRAMES(1));
clear ffn
ffn = mysort.h5.createVariableAndOrFile(P.outFile, [P.sessionName '/frame_numbers/last_fn'], [1 1], [1 1], 'H5T_NATIVE_INT');
ffn(1,1) = int32(FRAMES(end));
clear ffn
ffn = mysort.h5.createVariableAndOrFile(P.outFile, [P.sessionName '/frame_numbers/dataDims'], [1 2], [1 2], 'H5T_NATIVE_INT');
ffn(1,:) = int32(size(ntk.data));
clear ffn
if lL > 0
    ffn = mysort.h5.createVariableAndOrFile(P.outFile, [P.sessionName '/frame_numbers/missing_fns'], [lL 2], [lL 2], 'H5T_NATIVE_INT');
    ffn(:,:) = [firstMissingFR(:) L(:)];
else
    ffn = mysort.h5.createVariableAndOrFile(P.outFile, [P.sessionName '/frame_numbers/missing_fns'], [1 1], [1 1], 'H5T_NATIVE_INT');
    ffn(1,1) = int32(-1);
end
clear ffn
