function [S,NlogL,optimInfo]...
    =gmcluster(X,k,start,reps, CovType, SharedCov, RegV, FixedSigma, options)
...
%   $Revision: 1.1.8.1 $  $Date: 2010/03/16 00:20:38 $$
...
for t = 1:reps
    switch start
        case 'randsample'
            initPara = randInitParam(X,k, CovType, SharedCov,RegV, FixedSigma);
        case 'partition'
    ...
    try
        [S0,ll0,  optimInfo0] = gmcluster_learn...
            (X, k, initPara,  CovType, SharedCov, RegV, FixedSigma, options);
...
function [S,ll, optimInfo] = ...
    gmcluster_learn(X, k, initPara, CovType, SharedCov, regVal, FixedSigma, options)
...
for iter = 1:options.MaxIter;
    ...
    try
        log_lh=mysort.clustering.wdensity(X,S.mu, S.Sigma, S.PComponents, SharedCov, CovType);
        [ll,post] = mysort.clustering.estep(log_lh);
    catch ME
    ...
    if SharedCov %common covariance
        if ~isempty(FixedSigma)
            S.Sigma = FixedSigma;
            for j=1:k
                S.mu(j,:) = post(:,j)' * X / S.PComponents(j);
            end
        elseif CovType == 2  %full covariance
            S.Sigma = zeros(d,d);
            for j = 1:k
                S.mu(j,:) = post(:,j)' * X / S.PComponents(j);
    ...
    % Find MUs that are equal and move them away
    maxvar = max(S.Sigma(:));
    tolerance = maxvar/(1000*size(S.mu,2));
    move_mu = zeros(size(S.mu,1),1);
    for i=1:size(S.mu,1)
        for j=1:size(S.mu,1)
            if i~=j
                if norm(S.mu(i,:)-S.mu(j,:))<tolerance
                    move_mu(i) = 1; % move both? yes.
                    move_mu(j) = 1;
                end
            end
        end
    ...
    move_idx = move_mu==1;
    factor = 1-(iter/options.MaxIter);
    if any(move_idx)
        S.mu(move_idx,:) = S.mu(move_idx,:) + ...
            factor*maxvar/100*randn(sum(move_idx), size(S.mu,2));
    end
...
function initPara = randInitParam(X,k, CovType, SharedCov,RegV,FixedSigma)
[n,d] = size(X);
