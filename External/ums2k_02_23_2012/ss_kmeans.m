...
% get version number
matlabVerNo = ver;
matlabVerYear = str2num(matlabVerNo(1).Release(3:6)); %added ijones
...
for rep = 1:opts.reps                                 % TOTAL # REPETITIONS
	...
	for iter = 1:opts.divisions                       % # K-MEANS SPLITS
		...
		while (~(assign_converge | mse_converge))     % convergence?
            ...
            if matlabVerYear >= 2013
                
                for wave = 1:M
                    centroid(assigns(wave,rep),:) = centroid(assigns(wave,rep),:) + waves(wave,:);
                end
            else               
                % added by ijones for Matlab version <2013
                uniqueAssigns = unique(assigns(1:M,rep));
                for i=1:length(uniqueAssigns)
                    waveInds = multifind(assigns(1:M,rep),uniqueAssigns(i));
                    centroid(uniqueAssigns(i),:) =  sum(waves(waveInds,:),1);
