% function example2()
%
% Demonstrates optical model of the human eye
%
...
for i = 1 : nd
    ...
    %eye.rotate( [ 0 0 1 ], 30 * pi / 180 );
    bench.append( eye );
    
    % create some rays
    % rays_in = Rays( nrays, 'collimated', [ -20 0 0 ], [ 1 0 0 ], 1.2 * pupil_diameter, 'hexagonal' );
    rays_in = Rays( nrays, 'collimated', ray_start, ray_direction, 1.2 * pupil_diameter, 'hexagonal' );
...
bench.append( eye );
nrays = 40; 
rays_in = Rays( nrays, 'collimated', ray_start, ray_direction, 1.2 * pd( mi ), 'hexagonal' );
...
% ray_start = [ -(viewing_distance + 13.3) 0 0 ];
% ray_direction = [ 1 0 0 ];
% create some rays
rays_in = Rays( nrays, 'source', ray_start, ray_direction, 1.2 * pupil_diameter / (viewing_distance + 13.3), 'hexagonal' );
...
nrays = 40; 
rays_in = Rays( nrays, 'source', ray_start, ray_direction, 1.2 * pupil_diameter / (viewing_distance + 13.3), 'hexagonal' );
rays_in2 = Rays( nrays, 'source', [-20 0 0], [1 0 0], 1.2 * pupil_diameter / (viewing_distance + 13.3), 'hexagonal' );
rays_through = bench.trace( rays_in );
rays_through2 = bench.trace( rays_in2 );
bench.draw( rays_through, 'lines' );
bench.draw( rays_through2, 'lines' );
% bench.draw( [rays_through rays_through2], 'lines' );

[a,b] = rays_through( end ).stat
[a,b] = rays_through2( end ).stat
toc;
