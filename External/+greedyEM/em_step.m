function [W,M,R] = em_step(X,W,M,R,P,plo,S)
...
%  S - Covariance matrix, if you provide it, the covariance will not be
%      learned but set to S for all clusters (homoscedastic case)
%returns
   ...
   % update mixing weight
    ...
    if plo 
     if d == 1
      ...
      greedyEM.ellipse(2*sqrt(l(1)),2*sqrt(l(2)),phi,M(j,1),M(j,2),'k'); 
    end
   end
  end
end
