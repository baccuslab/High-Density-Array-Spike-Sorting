function [W,M,R,Tlogl] = em(X,T,kmax,nr_of_cand,plo,dia)
...
[W,M,R,P,sigma] = greedyEM.em_init_km(X,k,0);
sigma=sigma^2;
...
while 1
  ...
  while 1
    [W,M,R] = greedyEM.em_step(X,W,M,R,P,plo);
    if dia       fprintf('.');     end
    % likelihoods L (n x k) for all inputs and all components
    L = greedyEM.em_gauss(X,M,R);
 ...
 if test % average log-likelihood of test set
  ...
  [Mnew,Rnew,alpha] = greedyEM.rand_split(P,X,M,R,sigma,F,W,nr_of_cand); 
  if alpha==0
    if test % average log-likelihood of test set
      Ft = greedyEM.em_gauss(T,M,R) * W;
      Ft(Ft < eps) = eps;
  ...
  K                 = greedyEM.em_gauss(X,Mnew,Rnew);
  ...
  while ~done_here
    if dia         fprintf('*');    end
    [alpha,Mnew,Rnew] = greedyEM.em_step(X,alpha,Mnew,Rnew,Pnew,0);
    K    = greedyEM.em_gauss(X,Mnew,Rnew); Fnew = F*(1-alpha)+K*alpha;
  ...
  if logl <= veryoldlogl
    ...
    if test % average log-likelihood of test set
      Ft = greedyEM.em_gauss(T,M,R) * W; Ft(Ft < eps) = eps;
      Tlogl = [Tlogl; mean(log(Ft))];
    else Tlogl=0; end
    return;
  end
