...
for i = 1 : length([fileInfo.ChanList.number])
    if  [fileInfo.ChanList(i).kind] == 1
        ChanNr = fileInfo.ChanList(i).number;
        fileInfo.ChanList(i).sampleInt = SONGetSampleInterval(fid,(ChanNr));
        chanInfo = SONChannelInfo(fid, ChanNr);
        fileInfo.ChanList(i).scale = chanInfo.scale;
        fileInfo.ChanList(i).offset = chanInfo.offset;
    end
end
