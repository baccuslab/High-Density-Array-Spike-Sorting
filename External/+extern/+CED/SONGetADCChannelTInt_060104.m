...
% if (endTime > Info.maxChanTime/100000) || (isempty(endTime))
if (endTime > Info.maxChanTime * Info.divide/100000) || (isempty(endTime))
    disp('end time is too big! Using maximum of available data...');
   Tend = double((Info.maxChanTime * Info.divide/100000)/(FileH.usPerTime*FileH.timePerADC*FileH.dTimeBase));
   flag = 1;
else
   Tend = double(endTime / (FileH.usPerTime*FileH.dTimeBase));      % end time in clock tics
end

% dings = FileH.usPerTime*(1/FileH.dTimeBase);
% 
% if (endTime > Info.maxChanTime/100000) || (isempty(endTime))
%     disp('end time is too big! Using maximum of available data...');
%    Tend = double((Info.maxChanTime/100000)/(FileH.usPerTime*FileH.timePerADC*FileH.dTimeBase));
%    flag = 1;
% else
%    Tend = double(endTime / (FileH.usPerTime*1e-6));      % end time in clock tics
% %    Tend2 = double(endTime / (FileH.usPerTime*FileH.dTimeBase));      % end time in clock tics
% end
