...
%   see page: http://www.di.ens.fr/willow/SPAMS/
%   Note that OMP in SPAMS is the same as ORMP and java ORMP.
...
%   'paramSPAMS', optional parameter to use in mexLasso or mexOMP 
...
%  see: Dictionary Learning Experiments
%  see: Sparse signal representation using overlapping frames (proj02)
%  NORSIG 2003 paper: "Partial search vector selection for sparse signal 
%  representation."; http://www.ux.uis.no/~karlsk/proj02/norsig2003.pdf
% alternative functions:
%  see: GreedLab (sparsify), Thomas Blumensath et al. (Edinburgh)
%  see: SparseLab, David Donoho et al. (Stanford)
%  see: SPAMS, http://www.di.ens.fr/willow/SPAMS/
...
if isunix()   % catalog where SPAMS might be installed
...
if (strcmpi(met,'mexOMP') || strcmpi(met,'mexORMP') || ...
    ...
    %
    % check access to SPAMS
    if exist('mexOMP.m','file')
        % disp(['Access to mexOMP.m']);
    elseif exist([SPAMScatalog,'mexOMP.m'],'file')
