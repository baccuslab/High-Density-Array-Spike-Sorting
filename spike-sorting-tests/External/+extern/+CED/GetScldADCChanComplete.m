function [fileInfo,varargout]= GetScldADCChanComplete(filename,varargin)
import extern.CED.*;
...
% and a variable list of channels from which data should 
% be read (varargin). 
% 
...
% Read the complete channel

for m = 1 : length(selectChan)
    rawData = SONGetADCChannel(fid,selectChan{m});
