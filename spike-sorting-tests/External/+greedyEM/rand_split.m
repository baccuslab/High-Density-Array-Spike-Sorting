...
for i=1:k
  ...
  if length(XI) > 2*d  % generate candidates for this parent
    while j < nr_of_cand  % number of candidates per parent component
      ...
      if d==1
        ...
        cl = greedyEM.sqdist( Xloc', Xloc(r,:)' ); 
        [tmp,cl] = min(cl,[],2);
      end
      for guy = 1:2
        data = Xloc( cl==guy , :); 
        if size(data,1) > d
          ...
          Ws   = [Ws W(i)/2];
          Knew = zeros(n,1);
          Knew(XI) = greedyEM.em_gauss(Xloc,Mus(end,:),Covs(end,:));
          K = [K Knew];
        end
      end
    end
  ...
  last=size(Mus,1); if last>start % if candidates were added, do local partial EM
    ...
    iter=1;
    while ~done
      [alpha,Mnew,Rnew] = greedyEM.em_step_partial(Xloc,alpha,Mnew,Rnew,Pnew,n,0); 
      K2 = greedyEM.em_gauss(Xloc,Mnew,Rnew); 
