...
while k <= kmax
 ...
 if 1-Wnew/Wold < THRESHOLD*(10-9*(k==kmax))
	...
	Wnew        = sum(Dwin);Er=[Er; Wnew];
        if ~isempty(T); tmp=greedyEM.sqdist(T',M'); TEr=[TEr; sum(min(tmp,[],2))];end;
      end
    else
      k = kmax+1;
    end  
 ...
 Er=[Er; Wnew];
 if ~isempty(T); tmp=greedyEM.sqdist(T',M'); TEr=[TEr; sum(min(tmp,[],2))]; Er=[Er TEr];end;
M(kill,:)=[];
