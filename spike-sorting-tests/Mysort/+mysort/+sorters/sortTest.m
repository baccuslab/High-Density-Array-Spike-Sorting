...
%% INIT
pd = pdefs;
dpath = fullfile(pd.serverData, 'Espen', 'Benchmark', 'savedata_polytrode4_Thu_Apr_4_22_42_11_2013_c12b68c53c84418fbcf2052ee915be48');
if 1
    outPath = fullfile(dpath, 'TestSortings');
else
    outPath = 'C:\LocalData\Espen\SortingTestOut';
end
sortingName = 'Test22';
fname = 'testdata_janelia.h5';

%% LOAD
X_unfil = double(hdf5read(fullfile(dpath, fname), '/data')');
samplesPerSecond = double(hdf5read(fullfile(dpath, fname), '/samples_per_second'));
b  = mysort.mea.filter_design_fir(250, 7000, samplesPerSecond, 101);
X = conv2(X_unfil,b(:),'same');
epos = double(hdf5read(fullfile(dpath, fname), '/electrode_pos')');
epos = epos(:,2:3);
enrs = 1:size(epos,1);
gdf = double(hdf5read(fullfile(dpath, fname), '/ground_truth_spike_trains')');
