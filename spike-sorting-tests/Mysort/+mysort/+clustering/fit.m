function [obj BIC AIC] = fit(X,k,varargin)
...
pnames = {      'start' 'replicates'  'covtype' 'sharedcov'  'regularize'  'fixedsigma' 'options'};
dflts =  { 'randSample'           1      'full'      false             0         []         []};
[eid,errmsg,start,reps, CovType,SharedCov, RegV, FixedSigma, options] ...
...
try
    [S,NlogL,optimInfo] =...
        mysort.clustering.gmcluster(X,k,start,reps,CovType,SharedCov,RegV,FixedSigma,options);
    ...
    nParam = nParam + k-1 + k * d;
    BIC = 2*NlogL + nParam*log(n);
