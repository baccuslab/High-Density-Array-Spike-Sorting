classdef CMOSMEASession < mysort.ds.FilteredDataSourceInterface & ...
    ...
    methods
        %------------------------------------------------------------------
        function self = CMOSMEASession(fname, h5info_var, filterFactory_, session_idx, useFilter)
            ...
            self = self@mysort.ds.FilteredDataSourceInterface(filterFactory_, useFilter, 'CMOSMEASession', s_per_sec, ME);
            self = self@mysort.ds.ExtendedDataSourceInterface('CMOSMEASession', s_per_sec, ME);
        ...
        function wf = getWaveform_(self, nCut, channelIndex, varargin)
            ...
            wf = self.h5matrix_raw.getWaveform_(nCut, channelIndex, varargin{:});
            wf = double(wf)*self.lsb;
        end
